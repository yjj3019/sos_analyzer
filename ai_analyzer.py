...
class AIAnalyzer:
    ...
    def perform_ai_analysis(self, prompt: str, is_news_request: bool = False) -> Any:
        ...
        try:
            ...
            if is_news_request:
                llm_log_path = self.output_dir / "llm.txt"
                with open(llm_log_path, 'w', encoding='utf-8') as f:
                    f.write("--- PROMPT FOR SECURITY NEWS ---\n")
                    f.write(prompt)
                    f.write("\n\n--- LLM RESPONSE ---\n")
                print("\n--- LLM에게 보낸 보안 뉴스 프롬프트 ---")
                print(prompt)
...
## 시스템 기본 정보
- 호스트명: {system_info.get('hostname', 'N/A')}
- 운영체제: {system_info.get('os_version', 'N/A')}

## 분석 요청
다음 JSON 형식으로 종합적인 시스템 분석을 제공해주세요:
```json
{{
...
```
응답은 반드시 유효한 JSON 형식이어야 하며, 마크다운 코드 블록(```)으로 감싸주세요."""
        return prompt
    ...
    def _parse_ai_response(self, ai_response: str) -> Any:
        ...
        try:
            ...
            json_str = match.group(2) if match else ai_response
            return json.loads(json_str)
        except json.JSONDecodeError as e:
            raise ValueError(f"AI 응답 JSON 파싱 실패: {e}. 원본 응답 (일부): {ai_response[:300]}")
    ...
    def create_html_report(self, analysis_result: Dict[str, Any], sos_data: Dict[str, Any], graphs: Dict[str, str], output_dir: str, original_file: str) -> str:
        ...
        def create_table_rows(data_list, headers):
            ...
            for item in data_list:
                ...
                rows += "</tr>"
            return rows or f"<tr><td colspan='{len(headers)}' style='text-align:center;'>데이터 없음</td></tr>"
...
<head>
    ...
    <style>
        @import url('[https://cdn.jsdelivr.net/gh/projectnoonnu/noonfonts_six@1.2/S-CoreDream.css](https://cdn.jsdelivr.net/gh/projectnoonnu/noonfonts_six@1.2/S-CoreDream.css)');
        ...
        .info-table th, .info-table td, .data-table th, .data-table td {{ border: 1px solid #ddd; padding: 10px; text-align: left; }}
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                <h2>🛡️ 보안 뉴스</h2>
                <table class="data-table">
                    <thead><tr><th>CVE 식별자</th><th>심각도</th><th>보고일</th><th>요약</th><th>국내외 동향</th></tr></thead>
                    <tbody>{create_table_rows(security_news, ['cve_id', 'severity', 'public_date', 'summary', 'trends'])}</tbody>
                </table>
                <p style="font-size: 12px; text-align: center;">가이드된 보안 정보에 대한 확인은 <a href="[https://access.redhat.com/security/security-updates/security-advisories](https://access.redhat.com/security/security-updates/security-advisories)" target="_blank">Red Hat Security Advisories</a> 사이트에서 확인 필요.</p>
            </div>
        </div>
